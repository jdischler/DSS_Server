...
public final class Moving_Z_Window
{
	...
	private void calcWindowBounds() {
		...
		// Step 2: clamp window to the raster grid so we don't access array elements out of bounds
		if (mUpLeft_X < 0) {
			mUpLeft_X = 0;
		}
		if (mUpLeft_Y < 0) {
	...
	public final boolean advance() {
		...
		if (!mbShouldAdvance_Y) {
			...
			if (windowReal_X >= 0 && windowReal_X < mRasterWidth) {
//				for (int y = mUpLeft_Y; y <= mLowRight_Y; y++) {
				// NOTE: moving from the bottom (higher Y) value to the top (lower Y) value
				//	seems to be faster, possibly because it is more cache friendly?
				for (int y = mLowRight_Y; y <= mUpLeft_Y; y--) { // TODO: verify this works
					int cellValue = mRasterData[y][windowReal_X];
					if (cellValue != 0) {
						mTotal++;
